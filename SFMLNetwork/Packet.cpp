...
Packet& Packet::operator >>(int16_t& data)
{
    if (checkSize(sizeof(data)))
    {
        data = ntohs(*reinterpret_cast<const int16_t*>(&m_packetData[m_readPos]));
        m_readPos += sizeof(data);
    }
...
Packet& Packet::operator >>(uint16_t& data)
{
    if (checkSize(sizeof(data)))
    {
        data = ntohs(*reinterpret_cast<const uint16_t*>(&m_packetData[m_readPos]));
        m_readPos += sizeof(data);
    }
...
Packet& Packet::operator >>(int32_t& data)
{
    if (checkSize(sizeof(data)))
    {
        data = ntohl(*reinterpret_cast<const int32_t*>(&m_packetData[m_readPos]));
        m_readPos += sizeof(data);
    }
...
Packet& Packet::operator >>(uint32_t& data)
{
    if (checkSize(sizeof(data)))
    {
        data = ntohl(*reinterpret_cast<const uint32_t*>(&m_packetData[m_readPos]));
        m_readPos += sizeof(data);
    }
