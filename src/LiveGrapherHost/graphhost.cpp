...
int graphhost_t::sockets_readh( struct socketconn_t& conn ) {
    ...
    error = recv( conn.fd ,
                  (char*) conn.readbuf ,
                  conn.readbuflength - conn.readbufoffset ,
                  0 );
    if ( error < 1 ) {
...
int graphhost_t::sockets_readdoneh( uint8_t* inbuf ,
                                    size_t bufsize ,
                                    struct socketconn_t& conn ) {
    inbuf[15] = 0;
...
int graphhost_t::sockets_writeh( struct socketconn_t& conn ) {
    ...
    while ( 1 ) {
        ...
        conn.writebufoffset += send( conn.fd ,
                                     (char*) conn.writebuf ,
                                     conn.writebuflength - conn.writebufoffset ,
                                     0 );
 ...
 */
int graphhost_t::sockets_queuewrite( struct socketconn_t& conn ,
                                     uint8_t* buf ,
                                     size_t buflength ) {
    struct writebuf_t writebuf;
