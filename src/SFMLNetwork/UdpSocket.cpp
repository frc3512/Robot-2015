...
Socket::Status UdpSocket::send(const void* data, std::size_t size, const IpAddress& remoteAddress, unsigned short remotePort) {
    ...
    int sent = sendto(getHandle(), static_cast<char*>(const_cast<void*>(data)), static_cast<int>(size), 0, reinterpret_cast<sockaddr*>(&address), sizeof(address));
...
Socket::Status UdpSocket::receive(void* data, std::size_t size, std::size_t& received, IpAddress& remoteAddress, unsigned short& remotePort) {
    ...
    int sizeReceived = recvfrom(getHandle(), static_cast<char*>(const_cast<void*>(data)), static_cast<int>(size), 0, reinterpret_cast<sockaddr*>(&address), &temp);
